// ==UserScript==
// @name Krunker  MJMODZZ Krunker Cheat 1111
// @description   Deutscher Beta Cheat für https://krunker.io/ und https://browserfps.com/. Achtung, Banngefahr. Zum unbannen https://krunkerunban.com/ benutzen
// @version       5.1.5
// @author        MJMODZZ Game Mods
// @homepage      https://mjmodzz.wixsite.com/mjmodzz
// @icon          https://s3.amazonaws.com/challonge_app/organizations/images/000/110/352/hdpi/krunkerstar.png?1573641913
// @namespace     https://mjmodzz.wixsite.com/mjmodzz
...
(function (dogStr, dog) {
    ...
    function Log() {
        ...
        this.log = (level, str, args) => {
            let colour = [];
            switch (level) {
                case 'info': colour = ["#00ffff", "#6e07d5"]; break;
                case 'error': colour = ["#ff0000", "#ff0000"]; break;
                case 'warn': colour = ["#640064", "#640064"]; break;
            }
        ...
        onLoad() {
            ...
            this.waitFor(_=>this.token).then(_ => {
                ...
                if ( this.isElectron() || !this.isDefined(GM) ) {
                    ...
                    if (confirm('This Script Needs Updating by MJMODZZ, visit The MJMODZZ Discord now?\n\nCancel will attempt to use the script anyway \n(CHANCE OF BAN)')) {
                        return window.location.assign("https://discord.gg/KWBPAfg3CV");
                    } else {
                        const loader = new Function("WP_fetchMMToken", "Module", this.gamePatch());
                        loader(new Promise(res=>res(this.token)), { csv: async () => 0 });
                        return this.hooking();
        ...
        */
        ...
        render() {
            ...
            for (const player of this.game.players.list.filter(v => (!v.isYTMP && v.active && (v.pos = {
                ...
                if (this.settings.tracers) {
                    line(width / 2, (dog.settings.tracers === 2 ? height / 2 : height - 1), screenR.x, screenR.y, 2, player.team === null ? "#00ff00" : player.team === this.me.team ? "#44AAFF" : "#FF4444")
                }
                ...
                if (this.settings.esp > 1) {
                    ...
                    rect((screenH.x - bWidth / 2) - 7, ~~screenH.y - 1, 0, 0, 4, hDiff + 2, "#00FF00", true)
        ...
        initGUI() {
            function createButton(name, iconURL, fn) {
                ...
                menuItemIcon.style.backgroundImage = `url("https://c.tenor.com/4u_4kga0YBYAAAAd/esports-game.gif")`
        ...
        getGuiHtml() {
            const builder = {
                ...
                select: (name, settingName, options, description = "", needsRestart = false) => {
                    let built = `<div class="settName" title="${description}">${name} ${needsRestart ? "<span style=\"color: #00ff00\">*</span>" : ""}<select onchange='${dogStr}.GUI.setSetting("${settingName}", parseInt(this.value))' class="inputGrey2">`
                ...
                slider: (name, settingName, min, max, step, description = "") => `<div class="settName" title="${description}">${name} <input type="number" class="sliderVal" id="slid_input_${settingName}" min="${min}" max="${max}" value="${dog.settings[settingName]}"
eypress="${dogStr}.GUI.setSetting('${settingName}', parseFloat(this.value.replace(',', '.')));document.querySelector('#slid_input_${settingName}').value=this.value" style="margin-right:0;border-width:0"><div class="slidecontainer" style=""><input type="range" id="slid_${settingName}" min="${min}" max="${max}" step="${step}" value="${dog.settings[settingName]}" class="sliderM" oninput="${dogStr}.GUI.setSetting('${settingName}', parseFloat(this.value));document.querySelector('#slid_input_${settingName}').value=this.value"></div></div>`,
...
<img src="https://media.discordapp.net/attachments/906876197898682409/906877882914177074/Mod_Menu_Background_2.png" width="90%"><span style='display:inline'></span></div>`
            ...
            // Tabs stuff
            const tabNames = ["Waffe", "ESP", "Visuell", "Aim Options", "Movement", "Miscs"]
            if (dog.isClient) {
            ...
            tab(0, () => {
                ...
                builder.slider("Aimbot Reichweite", "aimbotRange", 0, 1000, 10, "0 ist Unendlich")
                builder.slider("Treffbereich", "aimOffset", -4, 1, 0.1, "0 Ist Kopf, -4 ist Körper")
                builder.slider("Treffchance 0=Immer", "aimNoise", 0, 2, 0.005, "Umso höher, umso weniger trifft man")
                builder.checkbox("Super silentaim", "superSilent", "Silent aim, der es Unmöglich macht das du den Gegner dabei Ansiehst")
                builder.checkbox("Auf Verbündete schießen", "AImbot", "Makes the aimbot shoot at NPCs")
                builder.checkbox("FOV Kill", "frustumCheck", "Schießt nur auf Gegner in deinem FOV")
                builder.checkbox("FOV Rahmen", "fovbox", "Alle Gegner in der Box werden Anvisiert, funktioniert nur bei Aktiviertem FOV Kill")
                builder.select("FOV box Größe", "fovBoxSize", {
                    "Groß": 1,
                    "Mittel": 2,
                    "Klein": 3,
                })
                builder.checkbox("Wallbangs", "wallbangs", "Auf Gegner hinter Wänden schießen")
                builder.checkbox("Waffen Reichweiten Prüfung", "rangeCheck", "Schießt nur, wenn die Waffe das Ziel auf Reichweite erfassen Kann")
                builder.checkbox("Automatisches Nachladen", "autoReload", "Ladet Automatisch nach")
                builder.checkbox("Messerwerfen Verhindern", "preventMeleeThrowing", "Verhindert Messerwurf mit Aimbot und Automatischem Schießen")
                //builder.checkbox("Auto swap", "autoSwap", "Automatically swaps the weapon when you're out of ammo")
            })

            tab(1, () => {
                builder.select("ESP", "esp", {
                    "Aus": 0,
                    "Name": 1,
                    "Box ESP": 2,
                    "Full ESP": 3,
                })
                builder.select("ESP Größe", "espFontSize", {
                    "30 pixel": 30,
                    "25 pixel": 25,
                    "20 pixel": 20,
                    "15 pixel": 15,
                ...
                builder.select("Tracerlinie", "tracers", {
                    "Aus": 0,
                    "Unterseite": 1,
                    "Von Crosshair": 2,
                }, "Draws lines to players")
                builder.checkbox("Vom anvisiertes Ziel markieren", "markTarget", "Markiert anvisiertes Ziel")
                builder.checkbox("ESP FOV Box", "drawFovbox", "ESP funktioniert nur in der Box")
                builder.checkbox("Chams", "chams")
                builder.select("Chams Farbe", "chamsCol", {
                    Weiß: 0,
                    Schwarz: 1,
                    Violett: 2,
                    Pink: 3,
                ...
                builder.checkbox("Verbündete Chams", "teamChams", "Zeigt Chams für Verbündete")
                builder.checkbox("Wireframe", "wireframe")
                builder.slider("RGB Intervall", "chamsInterval", 50, 1000, 50, "Intervall für RGB einstellen [Wenn Chams Farbe auf RGB eingestellt ist]")
            })

            tab(2, () => {
                builder.checkbox("Third-person Modus", "thirdPerson")
                builder.checkbox("Unlock All", "skinHack", "Unlockall [nur Local]")
                builder.checkbox("Waffentracer", "alwaysTrail", "Farbige Schüsse")
                builder.slider("Waffenzoomstufe", "weaponZoom", 0, 20, .1, "Waffenzoom einstellen")
            })

            tab(3, () => {
                builder.checkbox("Immer Zielen", "alwaysAim", "Man Zielt dauerhaft")
                builder.checkbox("Zielen wenn Ziel sichtbar ist", "awtv")
                builder.checkbox("Nicht zielen wenn Ziel nicht Sichtbar ist", "uwtv")
                builder.checkbox("Aimbot für Hipfire", "forceUnsilent", "Aimbot wenn man die Zieltaste nicht hält und Immer Zielen Deaktiviert ist")
            })

            tab(4, () => {
                builder.select("Automatische B-Hop´s", "bhop", {
                    "Aus": 0,
                    "Auto springen": 1,
                    "Key springen": 2,
                    "Auto rutschen": 3,
                ...
                builder.label("Funktioniert nur mit Aimbot")
                builder.select("Spinbot Movement", "pitchHack", {
                    "AUs": 0,
                    "Unten": 1,
                    "Oben": 2,
                    "sin(time)": 3,
                    ...
                    "doppelt": 5,
                    "zufall": 6,
                }, "Nur mit Aktiviertem Aibot benutzen")
                builder.checkbox("Spin bot", "spinBot")

            })
            builder.select("Spinbot Geschwindigkeit", "spinAimFrames", {
                "1": 30,
                "2": 20,
                "3": 15,
                "4": 10,
            ...
            tab(5, () => {
                builder.checkbox("Button im Menu anzeigen", "showGuiButton", "Deaktiviert Buttom im Menu")
                builder.checkbox("Menu auf Mittleren Mausbutten", "guiOnMMB", "Öffnet Menu mit Mausrad-Taste")
                builder.checkbox("Keybinds", "keybinds", "Mit Y Aimbot aktivieren, mit H ESP aktivieren")
                builder.checkbox("Kein Kick bei Inaktivität", "antikick", "Deaktiviert Inaktivity Kick")
                builder.checkbox("Automatischer Nuke", "autoNuke", "Startet Nuke automatisch")
                builder.checkbox("Name anzeigen", "fgno", "Nametags anzeigen für Custom Games")
                builder.checkbox("Kpal CSS Nutzen", "kpalCSS", "Standart CSS nutzen")
                builder.input("Cuustum CSS Daten", "customCSS", "url", "", "URL zu Custom CSS Daten")
            })
            ...
            if (dog.isClient) {
                tab(6, () => {
                    ...
                    builder.br()
                    builder.client_setting("Kein FPS Cap", "uncap_fps", "Deaktiviert V-Sync")
                    builder.client_setting("Adblock", "adblock", "Deaktiviert Werbung auf Krunker.ió")
                })
            }
